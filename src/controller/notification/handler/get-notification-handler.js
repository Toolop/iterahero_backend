...
const getNotifications = async (request, h) => {
	...
	try {
		...
		if (by_user_id && by_user_id == 1) {
			...
			const totalRows = await pool.query('SELECT * FROM public."notification" WHERE id_notification IN (SELECT id_notification FROM public."receive" WHERE id_user = $1)', [id_user]);
			totalPage = Math.ceil(totalRows.rowCount / size);
		}
